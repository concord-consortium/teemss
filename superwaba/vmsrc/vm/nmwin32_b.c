...
*/
#if _WIN32_WCE >= 212//Rapier devices
#define WCE_SIP 1
#include <aygshell.h>
#endif
...
static int32 getTimeStamp()
	{
	return (int32)(GetTickCount() & 0x3FFFFFFF);
	}

// ds@110 Windows has more functionality than the palm so put debugging info in a list in
// another window.

// debugging window functions
static TCHAR *pszDebugWndClassName = TEXT("WabaDebugClass");
static HWND g_hDebugWnd = NULL;
static HWND g_hDebugListBox = NULL;
#define IDC_DEBUG_LIST                  101
...
LRESULT CALLBACK DebugWndProc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
	switch(msg)
	{
		...
		case WM_CREATE:
			...
			g_hDebugListBox = CreateWindow(	//WS_EX_CLIENTEDGE,
									TEXT("LISTBOX"), // Class Name
									TEXT("Debug List"), // Button Text
									WS_CHILD | WS_VSCROLL | WS_HSCROLL | WS_VISIBLE
									| LBS_NOINTEGRALHEIGHT | LBS_DISABLENOSCROLL, // Style
									0,0, // position
									cr.right, cr.bottom, // size
									hWnd, // handle of the parent win
									(HMENU)IDC_DEBUG_LIST,
									lpcs->hInstance, 0); // 

			if( g_hDebugListBox != NULL)
			{
				SendMessage (g_hDebugListBox, LB_ADDSTRING, 0, 
                            (LPARAM) TEXT("Debug window enabled OK"));
			}
			return 0;
			}
	}
...
static void createDebugWindow() {
	...
	wc.lpszClassName = pszDebugWndClassName;
	if (!RegisterClass(&wc)) {
		return;
	}

	g_hDebugWnd = CreateWindow(pszDebugWndClassName, TEXT("Waba Debug"),
...
// guich waba 1.1
int debugY=0;
static int traceMinY=0;
static int traceMaxY=160;
static void debug(char *msg, int wait) // ds@110
{
	int iCurrItem;
#ifdef WINCE
	TCHAR * szMsg = NULL;
	...
	if (g_hDebugListBox != NULL) {
#ifdef WINCE
		szMsg = xmalloc((strlen(msg) + 1) * sizeof(TCHAR));
		if (szMsg == NULL) {
			iCurrItem = LB_ERR;
		} else {
			asciiToUnicode(msg, szMsg, strlen(msg)+ 1);
			iCurrItem = SendMessage (g_hDebugListBox, LB_INSERTSTRING, 0, 
		...
		iCurrItem = SendMessage (g_hDebugListBox, LB_INSERTSTRING, 0, 
                            (LPARAM) (LPCTSTR) msg);
#endif
		// keep the list box down to 200 lines!
		if (iCurrItem != LB_ERR && iCurrItem != LB_ERRSPACE) {
			iCurrItem = SendMessage (g_hDebugListBox, LB_GETCOUNT, 0, 0);
			if (iCurrItem != LB_ERR && iCurrItem > 200) {
				// get rid of the last item in the list
				SendMessage (g_hDebugListBox, LB_DELETESTRING, (WPARAM) iCurrItem - 1, 0);
			}
		}
#ifdef WINCE
		if (szMsg != NULL) {
...
static long FAR PASCAL MainWndProc(HWND hWnd, UINT msg, WPARAM wParam, LONG lParam)
	...
	switch(msg)
		...
		case WM_DESTROY:
			PostQuitMessage(0);
			return 0;
			// otherwise we just drop through to the default
		default:
			return DefWindowProc(hWnd, msg, wParam, lParam);
		}
...
static WObject startApp(char *cmdLine, BOOL *alreadyRunning)
	...
	nmStackSize = 300;
	classHeapSize = 20000;
	objectHeapSize = 20000; // guich: changed
	className = 0;
	*alreadyRunning = 0;
...
#define WOBJ_WindowHWnd(o) (objectPtr(mainWinObj))[_winHookOffset + 0].refValue 

WObject mainWinObj; // ds 1.1 - made a global variable
...
/*
	hWnd = WOBJ_WindowHWnd(mainWinObj);
	MessageBox(NULL, TEXT("Welcome to Waba"),
			TEXT("Hello"), MB_ICONEXCLAMATION);
