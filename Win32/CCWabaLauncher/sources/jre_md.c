...
JRE_GetDefaultSettings(JRESettings *set)
{
    ...
    while (*--bp != '\\') ;
    bp -= 4;
    if (bp < buf || _strnicmp(bp, "\\bin", 4) != 0) {
	return -1;
    }
    *bp = '\0';
    set->javaHome = (char *)_strdup(buf);
...
{
    char *cp = (char *)JRE_Malloc(strlen(dir) + sizeof(RUNTIME_LIB) + 8);
...
{
    char *cp = (char *)JRE_Malloc(strlen(dir) * 4 + 64);
 ...
 */
typedef jint (*initVMProc)(void *); 
jint
JRE_GetDefaultJavaVMInitArgs(void *handle, void *vmargs)
{
//    FARPROC proc = GetProcAddress((HMODULE)handle, "JNI_GetDefaultJavaVMInitArgs");
    initVMProc proc = (initVMProc)GetProcAddress((HMODULE)handle, "JNI_GetDefaultJavaVMInitArgs");
 ...
 */
typedef jint (*createVMProc)(JavaVM **,JNIEnv **,void *); 
jint
JRE_CreateJavaVM(void *handle, JavaVM **vmp, JNIEnv **envp, void *vmargs)
{
//    FARPROC proc = GetProcAddress((HMODULE)handle, "JNI_CreateJavaVM");
   createVMProc proc = (createVMProc)GetProcAddress((HMODULE)handle, "JNI_CreateJavaVM");
