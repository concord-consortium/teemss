...
public class PushButtonGroup extends Control
{
	...
	/** the button will be selected and de-selected immediatly, 
		acting like an real button */
	public static final byte BUTTON = 1;
	/** one click in the button will select it and another click will de-select it. 
	   perhaps, only one button can be clicked at a time */
	...
	/** not used. can be useful if you have a group of PushButtonGroup 
		and want to know easily which group was pressed. Used in the Keyboard class. */
	public int id;
   ...
   */
	public PushButtonGroup(String[] names, boolean atLastOne, int selected, 
						   int gap, int insideGap, int rows, boolean allSameWidth, byte type)
	{
	...
	protected void drawComponent(Graphics g)
	{
		...
		for (int i =0; i < names.length; i++){
			...
			// if allSameWidth, center the label in the button
			g.drawText(names[i], x+insideGap/2+(w-widths[i])/2, y); 
			g.clearClip();
			x += w + gap;
			if ((i+1)%cols == 0){
				x = 0;
	...
	public void onEvent(int type, int x, int y)
	{
		...
		switch (type)
			...
			case PenEvent.PEN_UP:
				...
				} else if (actLikeButton || (sel == -1 && !atLastOne)) {
					Vm.sleep(150); 
					setSelected(-1);
				}
				break;
