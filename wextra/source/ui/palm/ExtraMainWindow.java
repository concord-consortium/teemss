	...
	implements  org.concord.waba.extra.event.ActionListener
{
   ...
   */
	KeyboardDialog kbDialog = null;
	public void _postEvent(int type, int key, int x, int y, int modifiers, int timeStamp)
	{
		if(type == KeyEvent.KEY_PRESS &&
		   (key == 76000 || // KEYBOARD_ABC
			key == 76001 || // KEYBOARD_123 
			key == 76002)){ // KEYBOARD
			if(kbDialog != null){
				// this will call kbDialogClosed()
				kbDialog.hide();
			} else if(newFocus instanceof Edit ||
					  newFocus instanceof TextControl){
				if(newFocus instanceof Edit) kbDialog = new KeyboardDialog((Edit)newFocus, this);
				else kbDialog = new KeyboardDialog((TextControl)newFocus, this);
				setFocus(this);
				kbDialog.show();
				return;
			} 
