...
import waba.io.impl.ISerialPort;
...
ISerialPort portImpl; //mac implementation
...
*/
		...
		portImpl = waba.io.impl.SerialPortFactory.getISerialPort(number,baudRate,bits,parity,stopBits);
...
public boolean close()
	...
	if(portImpl == null) return false;

	return portImpl.close();
...
public int readBytes(byte buf[], int start, int count)
	...
	if(portImpl == null) return -1;

	return portImpl.readBytes(buf,start,count);
...
public boolean setFlowControl(boolean on)
	...
	return portImpl.setFlowControl(on);
...
public boolean setReadTimeout(int millis)
	...
	return portImpl.setReadTimeout(millis);
...
public int writeBytes(byte buf[], int start, int count)
	...
	return portImpl.writeBytes(buf,start,count);
