...
<redirect:write file="pdf/{../@name}_{@name}_TN-rtf.fo">
...
<xsl:template match="shared-image">
	<xsl:choose>
  		<xsl:when test="@screenshot='true'">
			...
			src="file:../images/Technical_Hints/screenshots/{@name}/RAW_{@name}.jpeg"/>
			</fo:block>
		</xsl:when>
		<xsl:otherwise>
			<fo:block text-align="center" border="solid 0.5pt black" margin-left="2in"
			...
			src="file:../images/Technical_Hints/pictures/{@name}/RAW_{@name}.jpeg"/>
			</fo:block>
		</xsl:otherwise> 
	</xsl:choose> 
      </xsl:template>
...
<xsl:template match="ext-image">
	...
	<fo:external-graphic  height="2.658in" width="3in"
		src="file:../images/{ancestor::unit/@name}/{ancestor::investigation/@name}/{@name}/RAW_{@name}.jpeg"/>
	</fo:block>
      </xsl:template>
