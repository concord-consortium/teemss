...
char *eval(char *p, long *valuePtr, char *refPtr, int *errorPtr)
{
    ...
    *refPtr = TRUE;
    ...
    while (TRUE) {
        ...
        if ( !p ) {
            return NULL;
        } else if (!backRef && (status > ERROR_RANGE || status == INCOMPLETE)) {
...
char *evalNumber(char *p, long *numberPtr, char *refPtr, int *errorPtr)
{
    ...
    *refPtr = TRUE;
    ...
    // and include letters, numbers, '.', '_', '$', '?', '@')
        ...
        if (FLookupSym(name, &cb)) {
            *numberPtr = cb;
            if (isNoBackrefLocalLabel(name)) {
                *refPtr = 0;
                return p;
            }
            if (gfPass2) {
...
int doOp(long val1, long val2, char op, long *result)
{
    ...
    case '.' : *result = val1 + val2;  return OK;   // PP
    case '-' : *result = val1 - val2;  return OK;
    case '&' : *result = val1 & val2;  return OK;
    case '|' : *result = val1 | val2;  return OK;
    case '>' : *result = val1 >> val2; return OK;
